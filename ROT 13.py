...
def process():
    if var_input.get() == "file":
        ...
        if isinstance(processed_text, str):
            ...
            result_text.delete('1.0', tk.END)
            result_text.insert(tk.END, "Processed text:\n" + processed_text)
    elif var_input.get() == "text":
        input_text = input_text_widget.get('1.0', tk.END)
        mode = var_mode.get()
        processed_text = process_text(input_text, mode)
        result_text.delete('1.0', tk.END)
        result_text.insert(tk.END, processed_text)
        btn_save.config(state="normal")  # Enable the save button
...
# Create main window
root = tk.Tk()
root.title("Rot13 Encryption/Decryption")
...
# Process Button
btn_process = tk.Button(root, text="Process", command=process, bg=SKY_BLUE)
